...
interface GenerationPlanPayload {
  ...
  functionsPlan: {
    ...
    plannedAdapterFiles: Array<{
      adapterName: string;
      filePath: string;
      methodCount: number;
    }>;
    connectorAdapterMappings: Array<{
      connectorKey: string;
      adapterName: string;
      adapterFilePath: string;
    }>;
    triggerStrategy: Array<{
      functionName: string;
      strategy: string;
    }>;
    handlerSignatures: Array<{
      functionName: string;
      exportedHandler: string;
      requestType: string;
      responseType: string;
    }>;
  ...
  infraPlan: {
    ...
    environmentParameterFiles: string[];
...
describe("power-exit generate functions command", () => {
  it("validates IR input and writes Azure Functions scaffold files", async () => {
    ...
    expect(await fileExists(path.join(generateOutput, "src/adapters/dataverseAdapter.ts"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "src/services/authContext.ts"))).toBe(true);
  ...
  it("supports --dry-run and writes only generation plan files", async () => {
    ...
    expect((plan.functionsPlan?.plannedAdapterFiles.length ?? 0) > 0).toBe(true);
    expect((plan.functionsPlan?.connectorAdapterMappings.length ?? 0) > 0).toBe(true);
    expect((plan.functionsPlan?.triggerStrategy.length ?? 0) > 0).toBe(true);
    expect((plan.functionsPlan?.handlerSignatures.length ?? 0) > 0).toBe(true);
    expect(plan.functionsPlan?.deploymentReadiness.scaffoldOnly).toBe(true);
    expect(plan.functionsPlan?.deploymentReadiness.needsConfig).toBe(true);
    expect(plan.functionsPlan?.deploymentReadiness.needsManualLogic).toBe(true);
  ...
  it("supports --clean for marker-tagged generated function files only", async () => {
    ...
    expect(await runCli(["analyse", inputFolder, "--out", analyseOutput], () => undefined, () => undefined)).toBe(0);
    await mkdir(path.join(generateOutput, "src/adapters"), { recursive: true });
    await writeFile(path.join(generateOutput, "host.json"), userOwnedHost, "utf-8");
    await writeFile(
      path.join(generateOutput, "src/adapters/httpAdapter.ts"),
    ...
    expect(await readFile(path.join(generateOutput, "src/adapters/httpAdapter.ts"), "utf-8")).toContain(
      "Generated by Power Exit."
    );
...
describe("power-exit generate infra command", () => {
  it("validates IR input and writes Azure infra scaffold files", async () => {
    const tempRoot = await createTempDirectory();
    const analyseOutput = path.join(tempRoot, "analyse-out");
    const generateOutput = path.join(tempRoot, "generate-infra-out");
    const inputFolder = path.resolve(
      process.cwd(),
      "packages/fixtures/samples/solutions/mixed-partial"
    );
    ...
    expect(
      await runCli(
        ["generate", "infra", path.join(analyseOutput, "ir.json"), "--out", generateOutput],
        stdOut.push.bind(stdOut),
        stdErr.push.bind(stdErr)
      )
    ).toBe(0);

    expect(await fileExists(path.join(generateOutput, "infra/main.bicep"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "infra/parameters.dev.json"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "infra/parameters.test.json"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "infra/parameters.prod.json"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "infra/modules/function-app.bicep"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "infra/modules/key-vault.bicep"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "infra/generation-report.md"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "infra/migration-notes.md"))).toBe(true);
    expect(stdOut[0]).toContain('"command":"generate-infra"');
    expect(stdOut[0]).toContain('"resourcesPlanned"');
    expect(stdOut[0]).toContain('"modulesPlanned"');
    expect(stdErr).toEqual([]);

    await rm(tempRoot, { recursive: true, force: true });
  });

  it("supports --dry-run and writes only generation plan files for infra", async () => {
    const tempRoot = await createTempDirectory();
    const analyseOutput = path.join(tempRoot, "analyse-out");
    const generateOutput = path.join(tempRoot, "generate-infra-out");
    const inputFolder = path.resolve(
    ...
    expect(await fileExists(path.join(generateOutput, "infra/main.bicep"))).toBe(false);
    expect(await fileExists(path.join(generateOutput, "generation-plan.json"))).toBe(true);
    expect(await fileExists(path.join(generateOutput, "generation-plan.md"))).toBe(true);

    const plan = await readGenerationPlan(generateOutput);
    expect(plan.infraPlan).not.toBeNull();
    expect((plan.infraPlan?.plannedResources.length ?? 0) > 0).toBe(true);
    expect((plan.infraPlan?.plannedModules.length ?? 0) > 0).toBe(true);
    expect(plan.infraPlan?.deploymentReadiness.scaffoldOnly).toBe(true);
    expect(plan.infraPlan?.deploymentReadiness.needsConfig).toBe(true);
    expect(plan.infraPlan?.deploymentReadiness.needsSecurityReview).toBe(true);

    await rm(tempRoot, { recursive: true, force: true });
  });

  it("skips conflicting infra files by default and overwrites with --force", async () => {
    const tempRoot = await createTempDirectory();
    const analyseOutput = path.join(tempRoot, "analyse-out");
    const generateOutput = path.join(tempRoot, "generate-infra-out");
    const inputFolder = path.resolve(
    ...
    const existingUserContent = "// user-owned infra template\n";

    expect(await runCli(["analyse", inputFolder, "--out", analyseOutput], () => undefined, () => undefined)).toBe(0);
    await mkdir(path.join(generateOutput, "infra"), { recursive: true });
    await writeFile(path.join(generateOutput, "infra/main.bicep"), existingUserContent, "utf-8");
    ...
    expect(await readFile(path.join(generateOutput, "infra/main.bicep"), "utf-8")).toBe(existingUserContent);
    const skippedPlan = await readGenerationPlan(generateOutput);
    expect(skippedPlan.skippedFiles.some((entry) => entry.path === "infra/main.bicep")).toBe(true);
    ...
    expect(await readFile(path.join(generateOutput, "infra/main.bicep"), "utf-8")).toContain("targetScope");
    const forcedPlan = await readGenerationPlan(generateOutput);
    expect(forcedPlan.overwrittenFiles.some((entry) => entry.path === "infra/main.bicep")).toBe(true);

    await rm(tempRoot, { recursive: true, force: true });
  });

  it("supports --clean for marker-tagged generated infra files only", async () => {
    const tempRoot = await createTempDirectory();
    const analyseOutput = path.join(tempRoot, "analyse-out");
    const generateOutput = path.join(tempRoot, "generate-infra-out");
    const inputFolder = path.resolve(
    ...
    const markerLines =
      "# Generated by Power Exit.\n# Do not edit directly unless you intend to own the generated file.\n";
    const userOwnedMain = "// user-owned infra template\n";

    expect(await runCli(["analyse", inputFolder, "--out", analyseOutput], () => undefined, () => undefined)).toBe(0);
    await mkdir(path.join(generateOutput, "infra/modules"), { recursive: true });
    await writeFile(path.join(generateOutput, "infra/main.bicep"), userOwnedMain, "utf-8");
    await writeFile(
      path.join(generateOutput, "infra/modules/app-service.bicep"),
      `${markerLines}\nlegacy module`,
      "utf-8"
    );
    ...
    expect(await readFile(path.join(generateOutput, "infra/main.bicep"), "utf-8")).toBe(userOwnedMain);
    expect(
